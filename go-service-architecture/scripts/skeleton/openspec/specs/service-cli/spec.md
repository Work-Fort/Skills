...
- REQ-019: The shutdown timeout SHALL default to 60 seconds and be configurable via the `--shutdown-timeout` flag (in seconds), the `NOTIFIER_SHUTDOWN_TIMEOUT` environment variable, or the `shutdown.timeout` YAML config key. The timeout governs the entire shutdown sequence including HTTP drain, runner wait, and store close.
...
### Scenario: Startup error is visible to the user
...
### Scenario: Graceful shutdown waits for in-flight jobs
...
### Scenario: Shutdown timeout prevents stuck jobs from blocking forever

- **Given** the daemon is running and a job is stuck (e.g., `@slow.com` 30-second delay)
- **When** a SIGTERM signal is received
- **Then** the daemon SHALL wait for in-flight jobs up to the configurable shutdown timeout (default 60 seconds)
